...
def get_pokemon_type_adv(pokemon, enemy_pokemon):
    ...
    pokemon_type_adv = []
    if enemy_pokemon.hp == 0.0 or pokemon.hp == 0.0: # ENEMY HAS FAINTED
    ...
    pokemon_type_adv = torch.tensor(pokemon_type_adv, dtype=torch.float).sum().item()
    # pokemon_type_adv = torch.tensor(normalize_score(pokemon_type_adv, -4.0, 2.0), dtype= torch.float)
...
def get_pokemon_move_adv(move, enemy_pokemon_list):
    move_adv = []
    if float(move.current_pp) == 0.0 or move.disabled:
        move_adv.append(0)
    else:
    ...
    s = torch.tensor(move_adv, dtype=torch.float).sum().item()
    move_adv = torch.tensor([normalize_score(s, -4.0, 2.0)], dtype=torch.float)
...
def get_pokemon_state(pokemon, enemy_pokemon_list, base, weather = None):
    ...
    if pokemon.hp != 0.0:
        ...
        s = torch.tensor(type_adv, dtype=torch.float).sum().item()
        type_adv = torch.tensor([normalize_score(s, -4.0, 2.0)], dtype=torch.float)
...
def get_state(game, player_identifier):
    '''return fully formed state made using game'''
